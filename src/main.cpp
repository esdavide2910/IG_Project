...
void InicializaOpenGLCuadro()
{
   ...
   constexpr float ancho_lineas = 1.0 ; // DUDA: ¿Hay que cambiar este valor a 1.0? ¿Por qué? Valor inicial: 2.1

   // sirve para leer el máximo ancho de puntos y lineas permitido por OpenGL
   GLfloat rango[2]; //DUDA: ¿De qué sirve esto?
   ...
   glGetFloatv( GL_SMOOTH_LINE_WIDTH_RANGE, rango ); 
   CError();
   ...
   // cout << "ancho de puntos deseado == " << ancho_puntos << endl ;
   // cout << "maximo ancho de puntos permitido " << rango[1] << endl ;
...
void FGE_PulsarLevantarTecla( GLFWwindow* window, int key, int scancode, int action, int mods )
{
   ...
   if ( glfwGetKey( window, GLFW_KEY_L) == GLFW_PRESS )
   {
      ...
      // .....
      if ( ProcesaTeclaFuenteLuz(escenas[ind_escena_act]->colFuentes(), key) == true )
         revisualizar_escena = true;
   ...
   switch ( key )
   {
      ...
      case GLFW_KEY_SLASH:         // tecla '-' en el teclado TKL
         // cout << "Haciendo Zoom OUT" << endl;
         camara->moverZ( +cam_d_incre_tecla );
         break;

      case GLFW_KEY_KP_ADD :        // tecla '+' en el teclado numérico 
      case GLFW_KEY_RIGHT_BRACKET:  // tecla '+' en el teclado TKL
         // cout << "Haciendo Zoom IN" << endl;
         camara->moverZ( -cam_d_incre_tecla );
         break;
