...
//AÑADIDO POR MÍ
void Objeto3D::getVerticesBordes(std::vector<Tupla3f> & vertices_borde)
{
...
/*--------------------------------------------------------------------------------------------------- */
/*                                   MIS FUNCIONES Y CLASES                                           */
/*--------------------------------------------------------------------------------------------------- */


void addNCirclePoints(int n, float r, float giro, std::vector<Tupla3f> & points)
{
   double step = ((M_PI * 2) / n);
   double x, y, current = step * giro;
   ...
   for(int i = 0; i < n; i++)
   {
      ...
      points.push_back( Tupla3f( x, y, 0.0 ) );

      current += step; 
   }
}

void displacePoints(std::vector<Tupla3f> & points, Tupla3f vector_desp)
{
   for(std::vector<Tupla3f>::iterator it = points.begin(); it != points.end(); ++it)
   {
      (*it)(0) += vector_desp(0);
      (*it)(1) += vector_desp(1);
      (*it)(2) += vector_desp(2);
